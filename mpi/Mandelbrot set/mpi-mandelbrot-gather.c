...
int main( int argc, char *argv[] )
{
    ...
    if ( argc > 1 ) {
        ysize = atoi(argv[1]);
        if (my_rank == 0 && ysize % comm_sz) {
            fprintf(stderr, "FATAL: this solution needs a height to be divisible by the number of processes. " \
                            "%d %% %d != 0\n", ysize, comm_sz);
    ...
    portion = xsize * ysize / comm_sz; /* How many pixels each proc is going to process*/
    ...
    draw_lines(my_rank*(ysize/comm_sz), (ysize/comm_sz)*(my_rank+1), partition_bitmap, xsize, ysize);

    MPI_Gather(partition_bitmap, portion*3, MPI_BYTE, bitmap, portion*3, MPI_BYTE, 0, MPI_COMM_WORLD);
