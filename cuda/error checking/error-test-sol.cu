...
int main(void)
{
    ...
    */
    ...
    CUDA_CHECK( cudaMemcpy(dA, &hA[0], sizeof(double)*N, cudaMemcpyHostToDevice ));
    CUDA_CHECK( cudaMemcpy(dB, &hB[0], sizeof(double)*N, cudaMemcpyHostToDevice ));
    ...
    CUDA_CHECK( cudaMemcpy(&hC[0], dC, sizeof(double)*N, cudaMemcpyDeviceToHost ) );
