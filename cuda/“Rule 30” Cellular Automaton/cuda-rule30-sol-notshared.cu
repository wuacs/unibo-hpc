...
__global__ void kernel_step_no_sharing( cell_t *cur, cell_t *next, int n )
{
    int idx = threadIdx.x + blockIdx.x * BLOCKSIZE + 1; 
    if ( idx < n - 1 ) {
        const cell_t left   = cur[idx-1];
        const cell_t center = cur[idx  ];
        const cell_t right  = cur[idx+1];
        next[idx] =
                ( left && !center && !right) ||
                (!left && !center &&  right) ||
                (!left &&  center && !right) ||
...
{
    const char *outname = "cuda-rule30-notshared.pbm";
    FILE *out;
    ...
    fprintf(out, "P1\n");
    fprintf(out, "# produced by cuda-rule30-sol-notshared.cu\n");
    fprintf(out, "%d %d\n", width, steps);
