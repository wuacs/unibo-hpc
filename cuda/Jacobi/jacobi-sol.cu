              ...
              double h2, int N)
{
    ...
    int i = threadIdx.x + blockIdx.x * blockDim.x; // Column index
    int j = threadIdx.y + blockIdx.y * blockDim.y; // Row index
...
{ 
    ...
    const int blocksize = 32; // Block is gonna be 2D so this is a side
    ...
    while (diff > tolerance && iterations < MAX_ITERATIONS) {
        ...
        CHECK_ERROR_MSG("Jacobi kernel broken");
        iterations += 2;
