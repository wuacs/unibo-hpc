...
__global__ void ker_compute_forces(circle_t * circles, int n, int * intersections, int circle) {

    int idx = threadIdx.x + blockIdx.x * blockDim.x + circle + 1;
    ...
    const float deltax = circles[idx].x - circles[circle].x;
    const float deltay = circles[idx].y - circles[circle].y;
    const float dist = hypotf(deltax, deltay);
    const float Rsum = circles[circle].r + circles[idx].r;
    if (dist < Rsum - EPSILON) {
        ...
        atomicAdd(&intersections[0], 1);
        circles[idx].dx += (overlap_x / K);
        circles[idx].dy += (overlap_y / K);
        atomicAdd(&circles[circle].dx, -(overlap_x / K));
        atomicAdd(&circles[circle].dy, -(overlap_y / K));
    }
...
int main( int argc, char* argv[] )
{
    ...
    intersections = (int*)malloc(sizeof(int));
    ...
    cudaSafeCall( cudaMalloc((void**)&d_intersections, sizeof(int)) );
    ...
    for (int it=0; it<iterations; it++) {
        ...
        for (int c = 0; c < n; c++) {
            grid.x = (n + c + MAXSIZEBLOCK - 1)/MAXSIZEBLOCK;
            ker_compute_forces<<<grid, block>>>(d_circles, n, d_intersections, c);
        }
        cudaSafeCall( cudaMemcpy(intersections, d_intersections, sizeof(int), cudaMemcpyDeviceToHost) );
...
#endif
        printf("Iteration %d of %d, %d overlaps (%f s)\n", it+1, iterations, *intersections, elapsed_iter);
    }
